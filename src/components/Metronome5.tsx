...
export const Metronome = () => {
  ...
  const [bpmKnobValue, setBpmKnobValue] = useState<number>(120);

  // Effect to handle sound buffer changes
  useEffect(() => {
    if (isPlaying && length && audioCtx) {
      // Restart the interval with the new sound buffers
      clearInterval(intervalRef.current!);
      isChap.current = false; // Reset the pattern
      if (audioCtx.chingBuffer) {
        playSound(audioCtx.chingBuffer); // Play initial ching
      }
      isChap.current = true; // Set up for next chap
      intervalRef.current = setInterval(play, length);
    }
  }, [audioCtx?.chingBuffer, audioCtx?.chapBuffer]); // Depend on both sound buffers
  ...
  const play = () => {
    ...
    if (!isChap.current && stopRequested.current) {
      ...
      const buffer = isChap.current ? audioCtx.chapBuffer : audioCtx.chingBuffer;
      if (buffer) {
        playSound(buffer);
      }
    }
  ...
  const startChingChap = () => {
    if (!tap1 || !tap2 || !audioCtx) return;
    const newInterval = Math.max(tap2 - tap1, 0);
    clearInterval(intervalRef.current!);
    isChap.current = false; // Reset pattern
    if (audioCtx.chingBuffer) {
      playSound(audioCtx.chingBuffer); // Play initial ching
    }
    isChap.current = true; // Set up for next chap
  ...
  // Rest of the JSX return remains the same...
  return (
